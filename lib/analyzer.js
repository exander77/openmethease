...
class RsDetector {
    detectors = []
    constructor(Rsnum, RsnumI, Genotype, GenotypeI, Genoset) {
        this.Rsnum = Rsnum
        this.RsnumI = RsnumI
        this.Genotype = Genotype
        this.GenotypeI = GenotypeI
        this.Genoset = Genoset
    ...
    process(chr, pos, rs, ref, vars, genotype) {
        ...
        //console.log('process', chr, pos, rs, ref, vars, genotype)
...
*/
        ...
        if (rs) {
            // It is Rsnum
            if (rs.match(/^rs/i)) {
                rsSNP = this.RsnumIndex[rs.replace(/^rs/i,'')]
            // It is Rsnum i
            } else if (rs.match(/^i/i)) {
                type = 'i'
                rsSNP = this.RsnumIIndex[rs.replace(/^i/i,'')]
            }
            // Try to find by position
            if (!rsSNP) {
                rsSNP = this.RsnumChrPosIndex[id]
            }
        }
        if (!rsSNP) return
        while (rsSNP.merged) {
        ...
        if (!rs && !vars) {
            ...
            if (rsSNP.referenceallele) {
                ...
                let nomags = [rsSNP.geno1, rsSNP.geno2, rsSNP.geno3].filter((e) => {
                    const gsid = makeid(type, rsSNP.rsid) + e
        ...
        const gsid = makeid(type, rsSNP.rsid, genotype)
        ...
        if (gtSNP) {
            ...
            const gsid = makeid(type, rsSNP.rsid, genotype_flipped)
            gtSNP = this.GenotypeIndex[gsid]
            if (gtSNP) {
...
class DnaAnalyzer {
    ...
    constructor(Rsnum, RsnumI, Genotype, GenotypeI, Genoset) {
        ...
        this.GenotypeI = GenotypeI
        this.Genoset = Genoset
        this.RsDetector = new RsDetector(Rsnum, RsnumI, Genotype, GenotypeI, Genoset)
    ...
    process(chr, pos, rs, ref, vars, genotype) {
        this.detectors.forEach(f => f.process(chr, pos, rs, ref, vars, genotype))
    }
    eval() {
        this.detectors.forEach(f => f.eval())
    }
...
class DnaAnalyzerVcf extends DnaAnalyzer {
    async run(textReader, log) {
        let count = 0
        ...
        while (!textReader.endOfStream) {
            ...
            if (vars[0] == '<NON_REF>') { // This is reference
                ...
                if (posindex == pos) this.process(chr, posindex, null, ref) 
            ...
            try {
                this.process(chr, pos, null, ref, vars, genotype)
            } catch (e) {
...
class DnaAnalyzer23andMe extends DnaAnalyzer {
    async run(textReader, log) {
        let count = 0
        ...
        while (!textReader.endOfStream) {
            ...
            const chr = data[1]
            const genotype = [...data[3]]
            const rs = data[0]

            try {
                this.process(chr, pos, rs, null, null, genotype)
            } catch (e) {
                console.error('Unexpected error occured:', e)
            }
...
module.exports.DnaAnalyzer = DnaAnalyzerVcf
module.exports.DnaAnalyzer23andMe = DnaAnalyzer23andMe
