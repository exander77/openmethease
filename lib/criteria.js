...
function wrap(text, value) {
    return '<b class="' + (value ? 'green' : 'red') + '">'+text+'</b>'
}
...
class Num {
    ...
    eval(table) {
        return this.cache = this.table.eval(this.type, this.id, this.args)
    }
    output(table) {
        return wrap(makeid(this.type, this.id, this.args), 
            this.cache !== undefined ? this.cache :this.table.eval(this.type, this.id, this.args))
    }
}
...
class Atleast extends Op {
    ...
    output(table) {
        return 'aleast('+this.count+','+this.operands.map(e => e.output(table)).join(', ')+')'
    }
} 
...
class And extends Op {
    ...
    output(table) {
        return 'and('+this.operands.map(e => e.output(table)).join(', ')+')'
    }
}
...
class Or extends Op {
    ...
    output(table) {
        return 'or('+this.operands.map(e => e.output(table)).join(', ')+')'
    }
}
...
class Not extends Op {
    ...
    output(table) {
        return 'not('+this.operands.map(e => e.output(table)).join(', ')+')'
    }
}
